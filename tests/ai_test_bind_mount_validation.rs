...
#[rstest]
fn test_bind_mount_validation_failure(
...
#[rstest]
fn test_bind_mount_validation_success(
    mut sandbox: SandboxManager,
) -> Result<()> {
    ...
    let _output1 =
        sandbox.run(&["--bind", &bind_mount, "echo", "first command"])?;

    // Run another command with the SAME bind mount - this should succeed
    let _output2 =
        sandbox.run(&["--bind", &bind_mount, "echo", "second command"])?;

    Ok(())
}

#[rstest]
fn test_bind_mount_options_validation(
    mut sandbox: SandboxManager,
) -> Result<()> {
    // Create temporary directory using the test filename pattern
    ...
    assert!(
        result.is_err(),
        "Expected bind mount validation to fail when options change"
    );
