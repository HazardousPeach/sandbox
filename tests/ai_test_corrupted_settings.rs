...
#[rstest]
fn test_get_or_create_with_corrupted_settings(
    ...
    assert!(
        ...
        "Settings file should exist after initial sandbox creation at {}",
    ...
    let write_cmd = std::process::Command::new("sudo")
        .args(&[
            "sh",
            "-c",
            &format!("echo '{}' > '{}'", corrupt_json, settings_file.display()),
        ])
        .output()?;

    if !write_cmd.status.success() {
        return Err(anyhow::anyhow!(
            "Failed to corrupt settings file with sudo: {}",
    ...
    assert!(
        !success,
        "Command should fail due to corrupted settings file"
    );
...
#[rstest]
fn test_get_or_create_with_unreadable_settings(
    mut sandbox: SandboxManager,
) -> Result<()> {
    // First, create a working sandbox
    let _output = sandbox.run(&["echo", "initial setup"])?;
    ...
    assert!(
        settings_file.exists(),
        "Settings file should exist after initial sandbox creation"
    );
    ...
    #[cfg(unix)]
    {
        ...
        if fs::set_permissions(&settings_file, perms).is_ok() {
            ...
            assert!(
                !success,
                "Command should fail due to unreadable settings file"
            );
            ...
            assert!(
                error_output
                    .contains("Failed to load existing sandbox settings")
                    || error_output.contains("The sandbox may be corrupted")
...
#[rstest]
fn test_get_or_create_with_directory_as_settings_file(
    mut sandbox: SandboxManager,
) -> Result<()> {
    // First, create a working sandbox
    let _output = sandbox.run(&["echo", "initial setup"])?;
    ...
    assert!(
        settings_file.is_dir(),
        "Settings path should now be a directory"
    );
    ...
    assert!(
        !success,
        "Command should fail when settings.json is a directory"
    );
