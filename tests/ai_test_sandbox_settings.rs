...
fn test_validate_against_config_network_change() -> Result<()> {
    ...
    assert!(
        error_msg.contains(
            "Please stop existing sandbox to change network settings"
        )
    );
...
fn test_validate_against_config_bind_mounts_removed() -> Result<()> {
    ...
    assert!(error_msg.contains(
        "Please stop the existing sandbox to change bind mount settings"
    ));
...
fn test_validate_against_config_multiple_changes() -> Result<()> {
    ...
    let settings =
        SandboxSettings::from_config(&original_config, &original_mounts);
